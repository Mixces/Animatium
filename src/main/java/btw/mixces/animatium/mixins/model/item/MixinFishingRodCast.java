...
public abstract class MixinFishingRodCast {
    ...
    private boolean animatium$getValue(boolean original, @Local(argsOnly = true) ItemDisplayContext displayContext) {
        if (AnimatiumClient.isEnabled() && AnimatiumConfig.instance().disableItemUsingTextureInGui && displayContext == ItemDisplayContext.GUI) {
            return false;
        } else {
            return (AnimatiumClient.isEnabled() && AnimatiumConfig.instance().oldFishingRodTextureStackCheck) || original;
        }
    }
}
