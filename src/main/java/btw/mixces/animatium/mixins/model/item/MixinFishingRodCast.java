...
public abstract class MixinFishingRodCast {
    ...
    private boolean animatium$getValue(boolean original, @Local(argsOnly = true) ItemDisplayContext displayContext) {
        if (AnimatiumClient.isEnabled() && AnimatiumConfig.instance().getDisableItemUsingTextureInGui() && displayContext == ItemDisplayContext.GUI) {
            return false;
        } else {
            return (AnimatiumClient.isEnabled() && AnimatiumConfig.instance().getOldFishingRodTextureStackCheck()) || original;
        }
    }
}
