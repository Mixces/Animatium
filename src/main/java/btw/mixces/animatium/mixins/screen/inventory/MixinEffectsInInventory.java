...
public abstract class MixinEffectsInInventory {
    ...
    private int animatium$oldEffectsInventoryPosition(AbstractContainerScreen<?> instance, Operation<Integer> original) {
        ...
        if (AnimatiumClient.isEnabled() && AnimatiumConfig.instance().getOldEffectsInventoryPosition() && !(this.screen instanceof AbstractRecipeBookScreen<?> recipeBookScreen && ((AbstractRecipeBookScreenAccessor) recipeBookScreen).getRecipeBookComponent().isVisible())) {
            return 0;
        } else {
            return imageWidth;
        }
