...
object AnimatiumParticles {
    ...
    private val DYNAMIC_REGISTRY =
        hashMapOf<SimpleParticleType, ParticleFactoryRegistry.PendingParticleFactory<SimpleParticleType>>()
    ...
    fun bootstrap() {
        val providerRegistry = ParticleFactoryRegistry.getInstance()
    ...
    private fun register(
        id: Identifier,
        alwaysSpawn: Boolean,
        provider: ParticleFactoryRegistry.PendingParticleFactory<SimpleParticleType>
    ): SimpleParticleType {
        val type = FabricParticleTypes.simple(alwaysSpawn)
        Registry.register(BuiltInRegistries.PARTICLE_TYPE, id, type)
